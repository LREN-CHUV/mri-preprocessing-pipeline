function [matlabbatch Session Opts prefixNIIf] = fMRI_automated_preproc(RootPath, SubjectID, varargin)
%--------------------------------------------------------------------------
...
warning('FIX NEEDED FOR PARTS LINES OF CODES 320-356 of function fMRI_automated_preproc.m')
warning('fMRI pipeline currently works when multiple subjects in RootPath, but when only one subject, subject to be processed need to be specified...')
...
%     fprintf('\nfMRI preprocessing of subject: %s\n',SubjectID) %# MODIFIED 2015-08-31 (Subj_folder replaced by NIIs)
    for s = 1:length(Subj_folder) %# MODIFIED 2015-08-31 (Subj_folder replaced by NIIs)
        temp = detectFolders(Subj_folder{s}); % detect experimental sessions within subject folder
        for r = 1:length(temp)
            NIIs_folder{s,r} = cellstr(strcat(repmat(Subj_folder{s},length(temp{r}),1),filesep,temp{r})); %# MODIFIED 2015-08-31 (Subj_folder replaced by NIIs)
        end
    end
...
for sub = 1:length(NIIs_folder) % all subjects
    ...
    [Sessions uniqueRes uniqueResIdx block CheckValidSub{sub}] = prepare_fMRI_session_coregStr2Fun(NIIs_folder{sub},Opts,CheckValid);
...
% if isempty(SubjectID)  %# MODIFIED 2015-08-31
