...
if nnz(bvals>1500)>29
    ...
    % Find MT file in subject directort. If no MT try MT_m (masked)
    MTfile = cellstr(pickfiles(dwDir.subjectDir,{'_MT.nii'},{'.'},{'.gz'}));
    if exist(MTfile{1},'file')
        StructuralTemplateFileName = MTfile{1};
    else MTfile = cellstr(pickfiles(dwDir.subjectDir,{'_MT_m.nii'},{'.'},{'.gz'}));
        if exist(MTfile{1},'file')
            StructuralTemplateFileName = MTfile{1};
        end
    end
