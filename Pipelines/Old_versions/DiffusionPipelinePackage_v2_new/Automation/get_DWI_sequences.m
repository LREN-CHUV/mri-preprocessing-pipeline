function [valid_protocols_diff,valid_protocols_fieldmap,ReadoutTimes] = get_DWI_sequences(InputFolder,ProtocolsFile)
...
ReadoutTimesBase = cellstr(get_protocol_names(ProtocolsFile,'__DWI__','[ReadoutTimes]')); % protocol name ..

valid_protocols_diff = {}; valid_protocols_fieldmap = {}; ReadoutTimes = [];
for i=1:length(diffusion_protocols)
    if exist([InputFolder,diffusion_protocols{i}],'dir')&&exist([InputFolder,fieldmap_protocols{i}],'dir')
        ...
        ReadoutTimes = [ReadoutTimes,ReadoutTimesBase(i)];
    end;
end;
