function [fmap_img,DWIunwarped]=DTI_unwarp(fm_imgs,anat_img,DWIimgs,pm_defs)
...
% Chloe Hutton
% $Id: B1Map_unwarp.m 10 2012-10-24 19:36:25Z nikolaus $
...
% If 3 images are parsed use only phase and 1st magnitude image
if size(fm_imgs,1)==3
    fm_imgs=fm_imgs(1:2,:);
end
...
unwarp_img{1}=FieldMap('Write',IP.epiP,IP.uepiP.dat,'unwarped',IP.epiP.dt(1),unwarp_info);
%----------------------------------------------------------------------
...
[X, Y, Z, numVols] = size(DWIimgs);

DWIunwarped=zeros(X,Y,Z,numVols);

% Need to register fm data to img data. Use registration params in unwarp
% code.
VG=spm_vol(char(anat_img));
VF=spm_vol(char(fm_imgs(2, 1).fname));

tM_params = spm_coreg(VG,VF(1,1));
tM = spm_matrix(tM_params(:)');
tM=IP.vdmP.mat\tM*IP.epiP.mat;

for V=1:numVols
    
    img_vol=squeeze(DWIimgs(:,:,:,V));
    
    out_img=FieldMap_4Dcorrection(img_vol,IP,tM);
    
    DWIunwarped(:,:,:,V)=out_img.dat;
