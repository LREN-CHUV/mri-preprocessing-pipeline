...
dwDir.PipelineBaseDir = fileparts(which('DWIInit.m'));
...
dwDir.mnB0Name         = fullfile(dwDir.dataDir,[dwDir.inBaseName '_b0.nii']);
...
if ~exist(dwDir.bvalsFile,'file')
    ...
    bval = dir('*bval*');
    if numel(bval(:,1))>1
        dwDir.bvalsFile = mrvSelectFile('r',{'*bval*';'*.*'},'Select bval file.');
    else
...
if ~exist(dwDir.bvecsFile,'file')
    ...
    bvec = dir('*bvec*');
    if numel(bvec(:,1))>1
        dwDir.bvecsFile = mrvSelectFile('r',{'*bvec*';'*.*'},'Select bvec file.');
    else
