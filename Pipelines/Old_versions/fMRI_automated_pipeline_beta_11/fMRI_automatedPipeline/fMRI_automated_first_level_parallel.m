function matlabbatch_first_level_par = fMRI_automated_first_level_parallel(Session,prefixNIIf,Opts)
%--------------------------------------------------------------------------
...
% number of subjects.
%
% prefixNIIf : string, prefix of preprocessed functional scans (to be used
...
%
%       - ModelFoldername : cell of cell of strings that specifies folder's
...
%
%       - ModelFilename : cell of cell of strings specifying paths to .mat file
...
for sub = 1:size(Session,1)
    ...
    matlabbatch_first_level_par{sub} = block_model_spec_est(Session(sub,1:size(Session,2)),Opts,prefixNIIf,matlabbatch,batchNum);
    
end

%% Save and run

fprintf('Jobs for parallel processing saved in %s\n',strcat(pwd, filesep, 'JOBS_PARALLEL_auto_preproc_', datestr(now,'yyyy-mm-dd-HH-MM'), '.mat'))
save(strcat(pwd, filesep, 'JOBS_PARALLEL_first_level_', datestr(now,'yyyy-mm-dd-HH-MM-SS-FFF'), '.mat'),'matlabbatch_first_level_par','Opts');
