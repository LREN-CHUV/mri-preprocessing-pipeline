function [Sessions uniqueRes uniqueResIdx block CheckValid] = prepare_fMRI_session_coregFun2Str(NIIs_folder,Opts,CheckValid)
    ...
    if ~isempty(temp)
        for i = 1:length(temp)
            ...
            if Opts.FilterAlreadyPreprocessedFMRI == 1
                ...
                if strcmpi(Opts.DirStructure,'LRENpipeline')
                    L3 = L3+length(char(temp2))+1;
                    L4 = L4+length(char(temp2))+1;
                end
                for j = 1:length(f)
