function [Sessions uniqueRes uniqueResIdx block CheckValid] = prepare_fMRI_session(NIIs_folder,Opts,CheckValid)
...
% % % TEMP TO GET B0 maps for Natacha's data (not only protocol for 64ch,
% should be equivalent)
temp = FilterFolders(FolderNames,'gre_field_mapping_1acq_rl.*',''); % normally we would use this B0 for EPI image distortion correction
% % %
...
%%%% STRUCTURAL
if ~isfield(Opts,'TokenStruct') % if no user-defined token
    if Opts.StructMT == 1
    ...
    temp = FilterFolders(FolderNames,['\w*' Opts.TokenStruct '\w*'],'');
    FlagMTfound = 0;
end
...
% Filtering of already preprocessed files:
...
if any(size(Structural)>1) % if still several structural scans
...
if strcmpi(Opts.DirStructure,'DICOMimport')
    if ~isempty(temp)
        for i = 1:length(temp)
            [f FileExt] = detect_nii_img_files(NIIs_folder,temp{i}); % select the files in the Structural folder
