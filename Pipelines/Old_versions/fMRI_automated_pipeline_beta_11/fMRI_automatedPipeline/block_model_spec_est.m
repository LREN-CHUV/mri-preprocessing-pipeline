...
%     data processing :
%
%       - AnalysisName (optional) : string (if not specified, 'GLM' is
%         used)
%
%       - DummyScans : cell of integers, indicating the number of dummy
%         volumes to discard from the GLM analysis
%
...
for Res = 1:length(Session) % ok here because 1 subject at a time, therefore index of resolution)
    ...
    if length(Session)<2
        ...
        if strcmpi(Opts.DirStructure,'DICOMimport')
            SubDir = fileparts(fileparts(Session{Res}.Struct{1}));
            NewDir = strcat(AnalysisName,'_',Session{Res}.EPIresolution{1});
        elseif strcmpi(Opts.DirStructure,'LRENpipeline')
            SubDir = fileparts(fileparts(fileparts(Session{Res}.Struct{1})));
            NewDir = strcat(AnalysisName,'_',Session{Res}.EPIresolution{1});
        else
    ...
    for sessnum = 1:nsess
        ...
        if Opts.DummyScans{Res}>0
            try
                MovParam{sessnum} = load(ForMovParamFilename{sessnum}); % the following line will fail if preprocessing is not already done and there were dummy scans before real fMRI acquisition
            catch Exception
                warning('fMRI data seem not be preprocessed yet (realignment parameter file not found). Because there were dummy scans during acquisition, first level cannot be setup at the moment')
                rethrow(Exception)
            end
            ...
            try
                ...
                warning('fMRI data seem not be preprocessed yet (realignment parameter file not found). However fMRI 1st level can be already prepared unless you want to perform RETROICOR')
            end
    ...
    for sessnum = 1:nsess
        ...
        if isfield(Opts,'CorrectPhysioNoise')
            ...
            R=cat(2,Physio{sessnum},RP{sessnum}); % can fail if no dummy scans but RETROICOR asked
    ...
    if sum(Check) == size(Session{Res}.EPI,2)
        ...
        for c = 1:size(Session{Res}.EPI,2)
            ...
            if strcmpi(Opts.DirStructure,'DICOMimport')
%    %TEMP MODIF%   block{batchNum+1}.spm.stats.fmri_spec.sess(c).multi = {strcat(fileparts(fileparts(Session{Res}.EPI{1}{1})), filesep, Opts.ModelFoldername{Res}{1}, Opts.ModelFilename{Res}{c}{1})}; % temporary modification to get multi-session designs with multiple subjects (RM, 2015-02-24)
...
% TWO LINES ABOVE TO BE DEBUGGED PROBABLY......... !!
            elseif strcmpi(Opts.DirStructure,'LRENpipeline')
