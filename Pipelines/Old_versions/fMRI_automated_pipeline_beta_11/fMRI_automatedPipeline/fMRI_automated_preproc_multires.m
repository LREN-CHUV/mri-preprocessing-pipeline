function [matlabbatch Session Opts prefixNIIf] = fMRI_automated_preproc_multires(RootPath, SubjectID, varargin)
%--------------------------------------------------------------------------
% Flexible automated pipeline for preprocessing of fMRI data using SPM
%--------------------------------------------------------------------------
%
%==========================================================================
% This one does the same as fMRI_automated_preproc, but coregister the
% functional to the structural instead of the reverse.
% It is more appropriate when dealing with fMRI runs with multiple
% resolutions.
% However use it with caution as it will take more time tochange the
% orientation matrices of more files (the functional volumes).
%==========================================================================
%
% Prerequisites:
%    -> SPM
...
%     %-------------------------------------------------------
%
%     However, please note that in the second hierarchy folder type, the
%     session level is currently not dealt and if it appears to be multiple
%     experimental sessions, only the first one will be processed (as
%     current of 2014-08-19.
%
%--------------------------------------------------------------------------
...
for sub = 1:length(NIIs_folder) % all subjects
    ...
    [Sessions uniqueRes uniqueResIdx block CheckValid] = prepare_fMRI_session_coregFun2Str(NIIs_folder{sub},Opts,CheckValid);
    ...
    if isempty(CheckValid)
        ...
        %% First part of preprocessing: realign (and unwarp), bias correct and coregister
        for res = 1:length(uniqueRes)
            ...
            [matlabbatch{batchNum} prefixNIIf batchNum] = block_coreg_funct_2_struct(Session{sub,res},Opts.RegisterToMean,prefixNIIf,batchNum);
        ...
        if strcmpi(SubjectID,'')
            save(strcat(NIIs_folder{sub}, filesep, 'WARNING_auto_preproc_', datestr(now,'yyyy-mm-dd-HH-MM-SS-FFF'), '.mat'),'CheckValid');
        else
            save(strcat(char(NIIs_folder{sub}), filesep, 'WARNING_auto_preproc_', datestr(now,'yyyy-mm-dd-HH-MM-SS-FFF'), '.mat'),'CheckValid');
        end
        prefixNIIf = [];
        prefixSub{sub} = prefixNIIf; % store all characters of the file prefix for further checking;
    end
...
if length(NIIs_folder) ~= sum(strcmp(prefixSub{1},prefixSub))
    ...
    if strcmpi(SubjectID,'')
        save(strcat(RootPath, filesep, 'WARNING_auto_preproc_', datestr(now,'yyyy-mm-dd-HH-MM'), '.mat'),'prefixSub');
    else
        save(strcat(char(Subj_folder), filesep, 'WARNING_auto_preproc_', datestr(now,'yyyy-mm-dd-HH-MM'), '.mat'),'prefixSub');
    end
end
...
if Opts.RunDARTEL == 1
    if sum(sum(cellfun(@isempty,Session))) % if only some (this will appear if not all subjects have same runs), return error
        ...
        if strcmpi(SubjectID,'')
            save(strcat(RootPath, filesep, 'WARNING_auto_preproc_', datestr(now,'yyyy-mm-dd-HH-MM-SS-FFF'), '.mat'),'CheckValid');
        else
            save(strcat(char(Subj_folder), filesep, 'WARNING_auto_preproc_', datestr(now,'yyyy-mm-dd-HH-MM-SS-FFF'), '.mat'),'CheckValid');
        end
    else
