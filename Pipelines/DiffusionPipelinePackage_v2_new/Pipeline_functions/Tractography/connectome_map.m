function mrtrixParams = connectome_map(Input_tck, connectome_output, mrtrixParams)
...
labelconfig_command = ['labelconfig ' mrtrixParams.qMRIpath '/freesurfer/mri/aparc+aseg.mgz ' mrtrixParams.config_in ' ' parcellation_output ' -lut_freesurfer ' mrtrixParams.LUTpath];

connectome_command_number = ['tck2connectome ' multitread_option force_option '-zero_diagonal ' Input_tck ' ' parcellation_output ' ' connectome_output '_number.csv'];
connectome_command_length = ['tck2connectome ' multitread_option force_option '-metric meanlength ' '-zero_diagonal ' Input_tck ' ' parcellation_output ' ' connectome_output '_length.csv'];
...
for i=1:length(qMRI_maps)
    ...
    connectome_commands_scalar{i} = ['tck2connectome ' multitread_option force_option '-metric mean_scalar -image ' qMRI_maps{i} ' -zero_diagonal ' Input_tck ' ' parcellation_output ' ' connectome_output '_' maptype '.csv'];
end
