...
if ~exist([outputdir filesep maskname],'file') || doMask==1
    ...
    gunzip([outputdir filesep '*mnb0*.gz'])
    pause(2)
%     delete([outputdir filesep '*.gz'])
    brain_mask([outputdir filesep mnb0name],[outputdir filesep maskname],TPMfilename);
end
brainmask=[outputdir filesep maskname];
...
if ~exist([outputdir filesep 'WestinMetrics'],'dir')
    mkdir([outputdir filesep 'WestinMetrics'])
end

% Make folder for QA maps
if ~exist([outputdir filesep 'Residuals'],'dir')
    mkdir([outputdir filesep 'Residuals'])
end
...
dtiWriteNiftiWrapper(ResidualMaps, rawDWIdata.qto_xyz, [outputdir filesep 'Residuals' filesep filename '_Resmap.nii'], 1, '', [],[],[],[], TR);
dtiWriteNiftiWrapper(mnResMap, rawDWIdata.qto_xyz, [outputdir filesep 'Residuals' filesep filename '_mnResmap.nii'], 1, '', [],[],[],[], TR);
dtiWriteNiftiWrapper(costfun_changemap, rawDWIdata.qto_xyz, [outputdir filesep 'Residuals' filesep filename '_CostfunChange.nii'], 1, '', [],[],[],[], TR);
