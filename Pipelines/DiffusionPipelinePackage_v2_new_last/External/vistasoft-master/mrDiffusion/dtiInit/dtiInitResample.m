...
if (doResamp && dwParams.clobber == -1) ...
        || dwParams.clobber == 1 ...
        || (~exist(dwDir.dwAlignedRawFile,'file') && (~exist([dwDir.outBaseDir '_unwarped.nii'],'file')))
    doRawResample = true;
else
    % If clobber = 'ask' prompt user to overwrite resampled data
    if dwParams.clobber == 0 
        resp = questdlg(['Preprocessed DWI dataset exists- would you like to overwrite it?'],...
