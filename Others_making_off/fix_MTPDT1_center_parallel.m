function fix_MTPDT1_center_parallel(InputFolder)
...
B1Sequence = {'al_B1mapping'};
B0Sequence = {'Field_mapping'};
...
PipelineFunction = 'fix_MTPDT1_center'; % fix_MTPDT1_center(InputFolder,SubjID,MTSequence,PDSequence,T1Sequence)
for i=1:Ns
    SubjID = SubjectFolders{i};
    JobID = ['job_',check_clean_IDs(SubjID)];
    InputParametersF = horzcat({InputFolder},{SubjID},{MTSequence},{PDSequence},{T1Sequence},{B1Sequence},{B0Sequence}); %#ok
    NameField = 'Name'; %#ok
