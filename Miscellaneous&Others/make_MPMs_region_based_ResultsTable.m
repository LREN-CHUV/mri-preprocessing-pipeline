function [OutputXLSFileName,Vol_data_xls,R2s_data_xls,MT_data_xls,PD_data_xls,R1_data_xls] = make_MPMs_region_based_ResultsTable(NeuroMorphoInputFolder,ProtocolsFile,PreviousOutputXLSFileName,OutputXLSFileName,SubjectListFileName)
...
SubjXLSFile = pickfiles([NeuroMorphoInputFolder,MorphoFolders{1}],{'.xls'});
...
Vol_data_xls = cell(Ns+1,Nh); R2s_data_xls = cell(Ns+1,Nh);
MT_data_xls  = cell(Ns+1,Nh); PD_data_xls  = cell(Ns+1,Nh); 
R1_data_xls  = cell(Ns+1,Nh); Nvox_data_xls = cell(Ns+1,Nh); Vol_norm_data_xls = cell(Ns+1,Nh); 
% Adding Header
Vol_data_xls(1,:)=Table_Header; R2s_data_xls(1,:)=Table_Header;
MT_data_xls(1,:)=Table_Header; PD_data_xls(1,:)=Table_Header; 
R1_data_xls(1,:)=Table_Header; Nvox_data_xls(1,:)=Table_Header; Vol_norm_data_xls(1,:)=Table_Header; 
s = 1;
for i=1:Ns
    ...
    Nxls = size(pickfiles(SubjFolder,{'Neuromorphics_Vols_MPMs_global_values.xls'}),1);  % Number of excel files (*.xls)
    SubjVols = cell(Nxls,Nh); SubjR2s = cell(Nxls,Nh);
    SubjMT   = cell(Nxls,Nh);  SubjPD = cell(Nxls,Nh); 
    SubjR1 = cell(Nxls,Nh); SubjNvox = cell(Nxls,Nh);SubjVols_norm = cell(Nxls,Nh);
    c = 0; 
    for j=1:Nse
        ...
        for k=1:Np
            ...
            for t=1:Nr
                ...
                SubjXLSFile = pickfiles(SubjRepetitionFolder,{'Neuromorphics_Vols_MPMs_global_values.xls'});
                ...
                SubjMT(c,4:end) = OneTable(:,4)';
                SubjPD(c,1) = MorphoFolders(i); SubjPD(c,2) = {c}; SubjPD(c,3) = MPM_resol;
                SubjPD(c,4:end) = OneTable(:,5)';
                SubjR1(c,1) = MorphoFolders(i); SubjR1(c,2) = {c}; SubjR1(c,3) = MPM_resol;
                SubjR1(c,4:end) = OneTable(:,6)';
                SubjNvox(c,1) = MorphoFolders(i); SubjNvox(c,2) = {c}; SubjNvox(c,3) = MPM_resol;
                SubjNvox(c,4:end) = OneTable(:,7)';                 
                SubjVols_norm(c,1) = MorphoFolders(i); SubjVols_norm(c,2) = {c}; SubjVols_norm(c,3) = MPM_resol;
                SubjVols_norm(c,4:end) = OneTable(:,8)';               
            end;
        end;
    end;
    Vol_data_xls(s+1:s+Nxls,:) = SubjVols; R2s_data_xls(s+1:s+Nxls,:) = SubjR2s;
    MT_data_xls(s+1:s+Nxls,:) = SubjMT; PD_data_xls(s+1:s+Nxls,:) = SubjPD; R1_data_xls(s+1:s+Nxls,:)= SubjR1;
    PD_data_xls(s+1:s+Nxls,:) = SubjPD; Nvox_data_xls(s+1:s+Nxls,:)= SubjNvox; Vol_norm_data_xls(s+1:s+Nxls,:)= SubjVols_norm;
    s = s + Nxls;
end;
